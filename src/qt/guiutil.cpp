...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("synchrotroncoin"))
        return false;
...
{
    // Convert synchrotroncoin:// to synchrotroncoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("synchrotroncoin://"))
    {
        uri.replace(0, 12, "synchrotroncoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SynchrotronCoin.lnk";
}
...
{
    return GetAutostartDir() / "synchrotroncoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=SynchrotronCoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SynchrotronCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  synchrotroncoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SynchrotronCoin-Qt"));
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(254,67,80); color: rgb(21,21,21); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(254,67,80); border-left-style: solid; border-left-width: 6px; }"
